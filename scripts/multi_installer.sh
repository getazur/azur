...
# a one line clone-and-compile for azur:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/azur/azur/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/azur ]; then
            read -r -p "${1:-azur directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old azur directory" && echo
                    rm -rf "$PWD"/azur
                    ;;
                *)
                    _fail "azur directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir azur
        git clone -b master -q https://github.com/azur/azur azur   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd azur
    fi
}

_build_azur() {
    _note "Building azur from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/azur')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/azur')"
            ;;
    esac
...
_note "Look in 'azur/build/src/' for the executible binaries. See 'https://github.com/azur/azur' for more project support. Cowabunga!"
